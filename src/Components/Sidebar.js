...
function Sidebar() {
    ...
    const { socket, members, setMembers, rooms, setRooms, setCurrentRoom, currentRoom, privateMemberMsg, setPrivateMemberMsg } = useContext(AppContext);

    function joinRoom(room, isPublic = true) {
        if (!user) {
            return alert("please login to join");
        }
    ...
    function getRooms() {
        fetch("https://chatappmernstack.herokuapp.com/rooms")
            .then((res) => res.json())
                ...
                <ListGroup.Item key={member.id} style={{ cursor: "pointer" }} active={privateMemberMsg?._id == member?._id} onClick={() => handlePrivateMemberMsg(member)} disabled={member._id === user._id} >
                    <Row>
                        <Col xs={2} className="member-status">
                            <img src={member.picture} alt="picture" className="member-status-img" />
